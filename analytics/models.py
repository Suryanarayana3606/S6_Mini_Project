...
class Sales_Forecast(models.Model):
    ...
    mape = models.DecimalField(max_digits=5, decimal_places=2, help_text="Mean Absolute Percentage Error")

    def __str__(self):
        return f"Forecast for {self.product.product_name} on {self.forecast_date}"
...
class FM_Customer_Segment(models.Model):
    ...
    def __str__(self):
        return f"Segment for {self.customer.customer_name}: {self.segment}"
